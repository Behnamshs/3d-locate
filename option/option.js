...
document.querySelectorAll(".like-btn").forEach(btn => {
  btn.addEventListener("click", e => {
    e.stopPropagation(); // جلوگیری از اجرا روی عکس
...
// انتخاب آیتم لایک
const heartItem = document.querySelector(".action .item:nth-child(2) i"); // آیکون قلب

heartItem.addEventListener("click", e => {
  e.stopPropagation(); // جلوی فعال شدن روی کارت
  const parentItem = heartItem.parentElement; // .item مربوطه
  parentItem.classList.toggle("heart-active");
});